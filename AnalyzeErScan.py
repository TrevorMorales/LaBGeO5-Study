...
for Folder in os.listdir(Parent):
    ...
    Axes = plt.gca()
    PlotColor = Axes.pcolormesh(XAxis,YAxis,CenterOfGravityEr.squeeze(), cmap = plt.get_cmap('viridis'))
    ColorBar = plt.colorbar(PlotColor)
    ...
    for i in range(StepsY):
        for j in range(StepsX):
            if(CrystalDetection[i][j] == 0):
                GlassAverage = GlassAverage + CenterOfGravityEr[i,j]
                GlassCount = GlassCount + 1
            if(CrystalDetection[i][j] == 1):
                CrystalAverage = CrystalAverage + CenterOfGravityEr[i,j]
            ...
            if(CrystalDetection[i][j] == 1):
                ...
                if(i < StepsY - 1 and j < StepsX - 1):
                    if(CrystalDetection[i+1][j] == 0 or CrystalDetection[i-1][j] == 0 or CrystalDetection[i][j+1] == 0 or CrystalDetection[i][j-1] == 0 or CrystalDetection[i][j-1] == 0):
                        ...
                        EdgeCrystalAverage = EdgeCrystalAverage + CenterOfGravityEr[i,j]
                    else:
                        InnerCrystalDetection[i][j] = 2
                        InnerCrystalCount = InnerCrystalCount + 1
                        InnerCrystalAverage = InnerCrystalAverage + CenterOfGravityEr[i,j]
    ...
    for i in range(StepsY):
        for j in range(StepsX):
            if(InnerCrystalDetection[i][j] == 3):
                InnerCrystalAverage = InnerCrystalAverage + CenterOfGravityEr[i,j]
                InnerCrystalAverage = InnerCrystalAverage + CenterOfGravityEr[i+1,j]
                InnerCrystalAverage = InnerCrystalAverage + CenterOfGravityEr[i,j+1]
                InnerCrystalAverage = InnerCrystalAverage + CenterOfGravityEr[i-1,j]
                InnerCrystalAverage = InnerCrystalAverage + CenterOfGravityEr[i,j-1]
